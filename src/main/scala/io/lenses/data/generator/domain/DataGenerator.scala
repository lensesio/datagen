...
abstract class DataGenerator[T <: AnyRef](implicit
    rf: RecordFormat[T],
    pbWriter: PBWriter[T]
) extends Generator
    with StrictLogging {
  ...
  private def generate[V](topic: String, delay: Long)(
      thunk: T => V
  )(implicit producer: KafkaProducer[String, V]): Unit = {
    Iterator.continually(generate()).flatten.foreach {
      case (k, v) =>
  ...
  override def avro(
    ...
    implicit val producer: KafkaProducer[String, Record] =
      new KafkaProducer[String, Record](props)
  ...
  override def json(
    ...
    implicit val producer: KafkaProducer[String, String] =
