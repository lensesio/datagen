...
object WeatherDataGenerator extends DataGenerator[WeatherS] {
  ...
  private def randWeather() = {
    ...
    val forecasts = (1 to 10).map { i =>
      ...
      ForecastS(
        ...
        Random.nextInt(100),
        Random.nextInt(),
        textValues(Random.nextInt(textValues.size))
      )
    }.toList
  ...
  private def randWeatherJ() = {
    ...
    val wind = WeatherOuterClass.Wind
      .newBuilder()
    ...
    val atmosphere = WeatherOuterClass.Atmosphere
      .newBuilder()
    ...
    val builder = WeatherOuterClass.Weather
      .newBuilder()
      .setWind(wind)
  ...
  override def protobuf(
    ...
    try {
      ...
      while (true) {
        cities.map { c => c -> randWeatherJ() }.foreach {
          case (k, v) =>
