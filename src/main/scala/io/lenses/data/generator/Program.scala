...
object Program extends App with StrictLogging {
  ...
  val parser = new scopt.OptionParser[Arguments]("generator") {
    ...
    opt[String]("brokers")
      ...
      .validate(
        f =>
          if (f.trim.isEmpty) Left("Invalid Kafka bootstrap brokers")
          else Right(())
      )
    ...
    opt[String]("schema")
      ...
      .validate(
        f =>
          if (f.trim.nonEmpty && Try(new URL(f.trim)).isSuccess) Right(())
          else Left("Invalid Schema Registry URL")
      )
    ...
    opt[String]("schema.mode")
      ...
      .text(
        "Schema registry mode. Use 'hw/hortonworks' to use the hortonworks serializers"
      )

    implicit val evFormatType: Read[FormatType] = new scopt.Read[FormatType] {
      override def arity: Int = 1

      override def reads: String => FormatType =
        a => FormatType.valueOf(a.trim.toUpperCase())
    }
    opt[FormatType]("format")
      .required()
      .action {
        case (v, a) =>
          a.copy(format = v)
    ...
    opt[Int]("data")
      .required()
      .action((v, a) => a.copy(dataSet = v))
      .validate(
        v =>
          if (v < 0 && v > 5)
            Left("Invalid data set option. Available options are 1 to 5")
          else Right(())
      )
      .text("""
          |Data set type.
          ...
          | 6 -  device temperature array
          | 7 -  subscriptions
          | 8 -  stations
          | 9 -  trips
          | 10-  customers (recursive schema)""".stripMargin)
  }

  parser.parse(args, Arguments("", 1, 1, -1, FormatType.JSON, "", "")).foreach {
    implicit arguments =>
      implicit val generatorConfig: DataGeneratorConfig = DataGeneratorConfig(
        arguments.brokers,
        arguments.schemaRegistry,
        arguments.produceDelay,
        arguments.defaultSchemaRegistry
      )
