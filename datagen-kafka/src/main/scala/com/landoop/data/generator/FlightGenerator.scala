...
object FlightGenerator extends App {

  val config = DataGeneratorConfig("localhost:9092", "http://localhost:8081", 1, true)
  val props = Producers.getAvroValueProducerProps(classOf[StringSerializer])(config)
  ...
  val schema = SchemaBuilder.record("flight").fields()
  ...
  for (k <- 1 to 5000000) {
    ...
    record.put("departure_time", startTime.plusMinutes(k).toInstant.toEpochMilli)
